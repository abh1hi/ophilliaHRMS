...
HRUser = Annotated[TokenPayload, Depends(require_role(UserRole.HR, UserRole.SUPER_ADMIN, UserRole.ADMIN))]
...
async def create_holiday(data: HolidayCreate, _user: HRUser, db: DB):
...
async def list_holidays(
    ...
    *,
    ...
    result = await db.execute(query.order_by(Holiday.date).offset(skip).limit(limit))
    return result.scalars().all()


@router.delete("/{holiday_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_holiday(holiday_id: UUID, _user: HRUser, db: DB):
    company_id = db.info.get("company_id")
