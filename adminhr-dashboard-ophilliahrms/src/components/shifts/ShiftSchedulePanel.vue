...
<template>
  <div class="space-y-6">
    <PageHeader title="Shift Schedules" subtitle="Define named recurring shift patterns" action-label="+ New Schedule" @action="openCreate" />
    <DataTable :columns="columns" :rows="rows" :loading="loading" :searchable="true" empty-text="No schedules defined yet.">
    ...
    <SlideDrawer :open="drawerOpen" :title="selected ? 'Edit Schedule' : 'New Shift Schedule'" width="w-full max-w-lg" @close="drawerOpen = false">
      <div class="space-y-5">
        <FormInput label="Schedule Name" :modelValue="form.name" @update:modelValue="form.name = $event" required placeholder="e.g. Week A Rotation" />
        <FormTextarea label="Description" :modelValue="form.description" @update:modelValue="form.description = $event" placeholder="Optional description" />
        <div class="grid grid-cols-2 gap-4">
          <FormInput label="Effective From" type="date" :modelValue="form.effective_from" @update:modelValue="form.effective_from = $event || undefined" />
          <FormInput label="Effective To" type="date" :modelValue="form.effective_to" @update:modelValue="form.effective_to = $event || undefined" />
        </div>
      </div>
      <template #footer>
    ...
    <ConfirmDialog :open="!!deleteTarget" title="Delete Schedule?" :message="`Delete '${deleteTarget?.name}'?`" :loading="deleting" @confirm="confirmDelete" @cancel="deleteTarget = null" />
  </div>
</template>
